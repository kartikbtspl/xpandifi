import { formatScheduleDate } from "../../util/Form-menu/DateUtils";
import {
  FaCalendarAlt,
  FaStore,
  FaTag,
  FaClock,
  FaBullseye,
  FaMoneyBillWave,
  FaChartLine,
  FaIndustry
} from "react-icons/fa";
import { Modal } from "../../components/ui/modal/Modal";

const CampaignInfoModal = ({ isOpen, onClose, campaign }) => {
  const targeting =
    campaign.devices?.length > 0
      ? campaign.devices.map((device) => device.name).join(", ")
      : "—";

  const infoItems = [
    { label: "Campaign Name", value: campaign.name || "—", icon: <FaTag /> },
    { label: "Brand", value: campaign.brandName || "—", icon: <FaIndustry /> },
    {
      label: "Schedule",
      value: formatScheduleDate(campaign.startDate, campaign.endDate) || "—",
      icon: <FaCalendarAlt />
    },
    { label: "Ad Type", value: campaign.adType || "—", icon: <FaBullseye /> },
    { label: "Store Type", value: campaign.storeTypes || "—", icon: <FaStore /> },
    { label: "Product Type", value: campaign.product || "—", icon: <FaTag /> },
    { label: "Targeting", value: targeting, icon: <FaBullseye /> },
    { label: "Duration", value: campaign.duration ? `${campaign.duration} sec` : "—", icon: <FaClock /> },
    {
      label: "Base Value",
      value: campaign.baseBid !== undefined ? `₹ ${campaign.baseBid}` : "—",
      icon: <FaMoneyBillWave />
    },
  ];

  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg" showCloseButton={true}>
      <div className="p-6 space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-semibold">{campaign?.name || "Campaign Details"}</h2>
        </div>

        {/* Info Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          {infoItems.map(({ label, value, icon }, index) => (
            <div key={index}>
              <label className="text-sm text-gray-600 flex items-center gap-2">
                
                {label}
              </label>
              <div className="mt-1 w-full px-3 py-2 text-gray-800 bg-white border border-gray-200 rounded-md shadow-sm text-sm">
                {value}
              </div>
            </div>
          ))}
        </div>
      </div>
    </Modal>
  );
};

export default CampaignInfoModal;












// import { formatScheduleDate } from "../../util/Form-menu/DateUtils";
// import {
//   FaCalendarAlt,
//   FaStore,
//   FaTag,
//   FaClock,
//   FaBullseye,
//   FaMoneyBillWave,
//   FaChartLine,
//   FaIndustry
// } from "react-icons/fa";

// const CampaignInfo = ({ campaign }) => {
//   const targeting =
//     campaign.devices?.length > 0
//       ? campaign.devices.map((device) => device.name).join(", ")
//       : "—";

//   const infoItems = [
//     { label: "Campaign Name", value: campaign.name || "—", icon: <FaTag /> },
//     { label: "Brand", value: campaign.brandName || "—", icon: <FaIndustry /> },
//     {
//       label: "Schedule",
//       value: formatScheduleDate(campaign.startDate, campaign.endDate) || "—",
//       icon: <FaCalendarAlt />
//     },
//     { label: "Ad Type", value: campaign.adType || "—", icon: <FaBullseye /> },
//     { label: "Store Type", value: campaign.storeTypes || "—", icon: <FaStore /> },
//     { label: "Product Type", value: campaign.product || "—", icon: <FaTag /> },
//     { label: "Targeting", value: targeting, icon: <FaBullseye /> },
//     { label: "Duration", value: campaign.duration ? `${campaign.duration} sec` : "—", icon: <FaClock /> },
//     // {
//     //   label: "Campaign Budget",
//     //   value: campaign.campaignBudget ? `₹ ${campaign.campaignBudget}` : "—",
//     //   icon: <FaMoneyBillWave />
//     // },
//     {
//       label: "Base Value",
//       value: campaign.baseBid !== undefined ? `₹ ${campaign.baseBid}` : "—",
//       icon: <FaMoneyBillWave />
//       // icon: <FaChartLine />
//     },
//     // {
//     //   label: "Max Bid",
//     //   value: campaign.maxBidCap ? `₹ ${campaign.maxBidCap}` : "—",
//     //   icon: <FaChartLine />
//     // },
//   ];

//   // Group items into pairs for two-column layout
//   const pairedItems = [];
//   for (let i = 0; i < infoItems.length; i += 2) {
//     pairedItems.push(infoItems.slice(i, i + 2));
//   }

//   return (
//     <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
//       <dl>
//         {pairedItems.map((pair, rowIdx) => (
//           <div
//             key={rowIdx}
//             className={`grid grid-cols-1 sm:grid-cols-2 gap-x-6 px-4 py-2 sm:px-5 sm:py-3 ${
//               rowIdx % 2 === 0 ? "bg-gray-50" : "bg-white"
//             }`}
//           >
//             {pair.map(({ label, value, icon }, idx) => (
//               <div key={idx} className="flex items-center">
//                 <dt className="flex items-center w-1/3 text-xs sm:text-sm font-medium text-gray-600">
//                   {icon && <span className="mr-2 text-gray-500 text-sm">{icon}</span>}
//                   {label}:
//                 </dt>
//                 <dd className="w-2/3 text-xs sm:text-sm text-gray-900">{value}</dd>
//               </div>
//             ))}
//           </div>
//         ))}
//       </dl>
//     </div>
//   );
// };

// export default CampaignInfo;








// import { formatScheduleDate, formatDate } from "../../util/Form-menu/DateUtils";

// const CampaignInfo = ({ campaign }) => {
//   console.log("Campaign Info Rendered", campaign);

//   // Devices as targeting string
//   const targeting =
//     campaign.devices && campaign.devices.length > 0
//       ? campaign.devices.map((device) => device.name).join(", ")
//       : "—";

//   const infoItems = [
//     { label: "Campaign Name", value: campaign.name || "—" },
//     { label: "Brand", value: campaign.brandName || "—" },
//     {
//       label: "Schedule",
//       value: formatScheduleDate(campaign.startDate, campaign.endDate) || "—",
//     },
//     { label: "Ad Type", value: campaign.adType || "—" },
//     { label: "Store Type", value: campaign.storeTypes || "—" },  
//     { label: "Product Type", value: campaign.product || "—" },
//     { label: "Campaign Budget", value: campaign.campaignBudget || "—" },
//     { label: "Base Bid", value: campaign.baseBid !== undefined ? `₹ ${campaign.baseBid}` : "—" },
//     { label: "Max Bid", value: campaign.maxBidCap ? `₹ ${campaign.maxBidCap}` : "—" },  
//     { label: "Targeting", value: targeting },
//     { label: "Status", value: campaign.isApproved || "—" },
//     { label: "Duration", value: campaign.duration ? `${campaign.duration} sec` : "—" },
//     // { label: "Created At", value: campaign.createdAt ? formatDate(campaign.createdAt) : "—" },
//     // { label: "Updated At", value: campaign.updatedAt ? formatDate(campaign.updatedAt) : "—" },
//   ];

//   return (
//     <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
//       {infoItems.map(({ label, value }, idx) => (
//         <div key={idx}>
//           <label className="text-sm text-gray-600">{label}</label>
//           <div className="mt-1 w-full px-3 py-2 text-gray-800 bg-white border border-gray-200 rounded-md shadow-sm text-sm">
//             {value}
//           </div>
//         </div>
//       ))}
//     </div>
//   );
// };

// export default CampaignInfo;